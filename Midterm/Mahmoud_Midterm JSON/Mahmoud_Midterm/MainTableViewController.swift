...
class MainTableViewController: UITableViewController {
    ...
    // Load from JSON
    let url = "https://creative.colorado.edu/~apierce/samples/data/restaurants.json"
    ...
    */
    ...
    func parsejson(_ data: Data)
    {
        ...
        do
        {
            let api = try JSONDecoder().decode([Restaurant].self, from: data)
//            print("API!!!!!!!!>>>>>>>", api)
            for result in api
            {
                print("$$$$",result)
                RealmService.shared.save(restaurant: result)
            }
